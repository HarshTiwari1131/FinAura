...
export default function IncomeList() {
  ...
  const filtered = useMemo(()=>{
    let rows = [...items]
    if (q) rows = rows.filter(r => (r.source || '').toLowerCase().includes(q.toLowerCase()))
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-end justify-between gap-3 flex-wrap">
        <div>
          <h2 className="text-2xl font-semibold text-white">Income Log</h2>
          <div className="text-sm text-slate-400">Review and manage your incomes</div>
        </div>
      ...
      <div className="rounded-xl border border-slate-700/60 bg-slate-800/60 backdrop-blur-sm p-4">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
          <input className="rounded-lg bg-slate-700/70 border border-slate-700 text-slate-50 px-3 py-2" placeholder="Search source" value={q} onChange={e=>setQ(e.target.value)} />
          ...
          <button onClick={()=>{setQ(''); setFrom(''); setTo('')}} className="btn-secondary">Reset</button>
        </div>
      </div>
        ...
        <table className="w-full text-sm">
          <thead className="bg-slate-800 text-slate-300">
            <tr className="text-left">
              ...
              <th className="px-4 py-2 border-b border-slate-700">Source</th>
